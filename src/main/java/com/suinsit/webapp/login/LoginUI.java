...
//		Ssorportalrol = builder.loadFromDataGrid("SSORPORTALROL");
//		Ssoruserol = builder.loadFromDataGrid("SSORUSEROL");
//		postInit();
//		logacces = builder.loadFromDataGrid("SSOLOGACCESS");
//		
//		 clsLogAccess = Thread.currentThread().getContextClassLoader().loadClass("org.suinsit.apps."+logacces.getNamespace()+"."+TextParser.getClassNameFormater(logacces.getCoentity()));
...
//		clsSsoruserol = Thread.currentThread().getContextClassLoader().loadClass("org.suinsit.apps."+Ssoruserol.getNamespace()+"."+TextParser.getClassNameFormater(Ssoruserol.getCoentity()));
//		clsSsorportalrol = Thread.currentThread().getContextClassLoader().loadClass("org.suinsit.apps."+Ssorportalrol.getNamespace()+"."+TextParser.getClassNameFormater(Ssorportalrol.getCoentity()));
	...
	private Entity getLog(boolean logout) {
		...
		if(logout) {
			logs.getField("LOGOUT").setValue(new java.sql.Timestamp(System.currentTimeMillis()));
		}else {
		...
		logs.getField("DISPOSITIVO").setValue(Executions.getCurrent().getBrowser());
		logs.getField("ACCESO").setValue(new java.sql.Timestamp(System.currentTimeMillis()));
		
		return logs;
	}
	private void failAccess(String message,String tipo) {
		Entity loge = getLog(false);
...
//					}
	...
	private void logout(boolean audit) {
		if(audit) {
			Entity loge = getLog(true);
